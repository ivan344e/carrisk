...
st.set_page_config(page_title="Clasificador para riesgo de vehiculos", layout="centered")
...
image_base64 = load_image_as_base64("imagen.png")
...
def main():
    ...
    # Título principal
    st.title("Clasificador de riesjo en vehiculos")
    #Titulo de Sidebar
    st.sidebar.header('Parametros en personas ')
 ...
 # Entradas del usuario en el Sidebar
    ...
    def user_input_features():
        ...
        option = ['family', 'sport', 'combi', 'minivan']
        Cartype = st.sidebar.selectbox('tipo de vehiculo', option)
    ...
    option = ['DT', 'Knn','NN']
    model = st.sidebar.selectbox('elija un modelo ?',option)

    st.subheader('Valores elejidos ')
    st.write(df)
    st.subheader(f'Modelo elejido: {model}')


    # Crear un botón para realizar la predicción
    if st.button('inicio de  Predicción'):
        
        if model == 'DT':
            Y_fut = modelTree.predict(df)
            resultado = labelencoder.inverse_transform(Y_fut)
            st.success(f'el resultado de la  predicción es: {clasify(resultado[0])}')
        elif model == 'Knn':
            #Normalización
            df[['age']] = min_max_scaler.transform(df[['age']])
            Y_fut = modelKnn.predict(df)
            resultado = labelencoder.inverse_transform(Y_fut)
            st.success(f'el resultado de la predicion  es: {clasify(resultado[0])}')
        else:
            #Normalización
            df[['age']] = min_max_scaler.transform(df[['age']])
